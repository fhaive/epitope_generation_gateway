...
FINAL_DIR_WITH_VAF = BASE / "input_final_epitope_tables_with_vaf"
FINAL_DIR_DEFAULT = BASE / "input_final_epitope_tables"
...
def plot_scatter(all_comp):
    ...
    outbase = PLOT_DIR / "Supplementary_Figure_S4_optional_original_vs_ppi_topology_ablated_rank_scatter"
    for ext in ["png", "svg", "pdf"]:
        fig.savefig(outbase.with_suffix(f".{ext}"), bbox_inches="tight")
...
def main():
    ...
    summary.to_csv(TABLE_DIR / "Supplementary_Figure_S4_source_data_by_sample.tsv", sep="\t", index=False)
    all_comp.to_csv(TABLE_DIR / "Supplementary_Figure_S4_detailed_candidate_ranks.tsv.gz", sep="\t", index=False, compression="gzip")
    ...
    cohort.to_csv(TABLE_DIR / "Supplementary_Figure_S4_cohort_summary.tsv", sep="\t", index=False)
    ...
    }]).to_csv(TABLE_DIR / "Supplementary_Figure_S4_GO_pathway_annotation_rank_impact_note.tsv", sep="\t", index=False)
    ...
    ]).to_csv(TABLE_DIR / "Supplementary_Figure_S4_weighting_scheme.tsv", sep="\t", index=False)
    ...
    plot_histogram(
        ...
        outbase=PLOT_DIR / "Supplementary_Figure_S4_panel_B_top10_overlap_ppi_topology_ablated_histogram",
    )
    ...
    plot_histogram(
        ...
        outbase=PLOT_DIR / "Supplementary_Figure_S4_panel_A_spearman_ppi_topology_ablated_histogram",
    )
