...
INPUT_BASE = BASE / "TCGA_melanoma"

FINAL_DIR = INPUT_BASE / "epitopes_prioritisation" / "final_epitopes"
PVAC_DIR = INPUT_BASE / "2A_somatic_mutation_epitopes" / "pvacSeq"
...
def read_pvacseq_vaf_lookup():
    ...
    pd.DataFrame(column_records).to_csv(TABLE_DIR / "Supplementary_Figure_S3_pvacseq_columns_used.tsv", sep="\t", index=False)
    pvac.to_csv(TABLE_DIR / "Supplementary_Figure_S3_pvacseq_vaf_rows_before_grouping.tsv", sep="\t", index=False)
    lookup.to_csv(TABLE_DIR / "Supplementary_Figure_S3_pvacseq_vaf_lookup_grouped.tsv", sep="\t", index=False)
...
def read_final_somatic_tables():
    ...
    pd.DataFrame(column_records).to_csv(TABLE_DIR / "Supplementary_Figure_S3_final_table_columns_used.tsv", sep="\t", index=False)
...
def draw_boxplot(matched, summary):
    ...
    png = PLOT_DIR / "Supplementary_Figure_S3_panel_A_vaf_all_somatic_vs_top10.png"
    svg = PLOT_DIR / "Supplementary_Figure_S3_panel_A_vaf_all_somatic_vs_top10.svg"
...
def draw_scatter(matched, summary):
    ...
    png = PLOT_DIR / "Supplementary_Figure_S3_panel_B_vaf_vs_rank_percentile_by_MHC.png"
    svg = PLOT_DIR / "Supplementary_Figure_S3_panel_B_vaf_vs_rank_percentile_by_MHC.svg"
...
def main():
    ...
    all_final.to_csv(TABLE_DIR / "Supplementary_Figure_S3_all_final_epitopes_with_rank_percentile.tsv", sep="\t", index=False)
    merged.to_csv(TABLE_DIR / "Supplementary_Figure_S3_somatic_candidates_with_vaf_all_match_attempts.tsv", sep="\t", index=False)
    matched.to_csv(TABLE_DIR / "Supplementary_Figure_S3_source_data_somatic_candidates_with_vaf.tsv", sep="\t", index=False)
    match_summary.to_csv(TABLE_DIR / "Supplementary_Figure_S3_vaf_match_summary.tsv", sep="\t", index=False)
    summary.to_csv(TABLE_DIR / "Supplementary_Figure_S3_summary_statistics.tsv", sep="\t", index=False)
